...
// structured `bonus` triple participate, and one with a `when` list is live
// while any of those modifiers is on (no `when` means always live).
// Prose-only abilities are informational and never returned here.
export const activeAbilities = (unit, modifiers) =>
  filter(
    unit?.abilities,
    (ability) =>
      ability.bonus &&
