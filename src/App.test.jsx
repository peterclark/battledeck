...
// Buttons built on usePressable (the reset buttons) tap on pointer up;
// plain buttons (modifiers, command cards) use click.
const tap = (el) => fireEvent.pointerUp(el);

// Calc-line terms split their value and code into differently sized spans,
// so exact getByText can't see the joined text — match the Term wrapper's
// full text content instead
const term = (text) => (_, element) =>
  element.classList.contains("Term") && element.textContent === text;
...
describe("App", () => {
  ...
  it("has no manual base controls — selection and cards set the numbers", () => {
    const { container, queryByLabelText } = setup();
    expect(queryByLabelText(/Add one die/)).not.toBeInTheDocument();
    expect(queryByLabelText(/rank/)).not.toBeInTheDocument();
  ...
  it("overkill shows when the unclamped total passes 5", () => {
    ...
    const plusOS = container.querySelector(".PlusOneOS");
    for (let i = 0; i < 7; i += 1) fireEvent.click(plusOS); // 0 OS +7 CC
...
describe("Units", () => {
  ...
  it("selection writes the calc line's base terms", () => {
    const utils = setup();
    const { dice } = utils;
    pickUnit(utils, "attacker", "Knights");
