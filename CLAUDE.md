...
The unit picker lists one row per fielded copy (`#1`/`#2`, damage shown) from both rosters; selection stores `attackerCopy`/`defenderCopy` alongside the uids plus `attackerArmy`/`defenderArmy` (`"mine"`/`"enemy"` — a mirror match can field the same unit on both sides, so uid+copy alone is ambiguous), and selecting an attacker copy — or closing the army builder with one selected — *prefills* the `inTheYellow`/`inTheRed` modifiers from that copy's marks (same prefill-not-lock philosophy as stats). Slots show the copy's `marked/total` and status.
