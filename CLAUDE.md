...
Undead copies also get a Reanimate button showing its Command Action cost (`reanimateCost` maps the `lesserUndead`/`majorUndead`/`greaterUndead` keywords to 1/2/3; anything else — including the Swarm of Rats, which prints no classification — returns null and renders no button). Tapping it heals one damage and locks that copy for the turn (`reanimated[uid][copyIndex]`, persisted with the roster); a destroyed or undamaged copy can't be raised. A tally line shows the Command Actions spent Reanimating this turn, reddening past the budget's allowance, and its **New turn** button clears the locks. The unit picker lists one row per fielded copy (`#1`/`#2`, damage shown); selection stores `attackerCopy`/`defenderCopy` alongside the uids, and selecting an attacker copy — or closing the army builder with one selected — *prefills* the `inTheYellow`/`inTheRed` modifiers from that copy's marks (same prefill-not-lock philosophy as stats). Slots show the copy's `marked/total` and status.
